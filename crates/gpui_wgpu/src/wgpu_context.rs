...
impl WgpuContext {
    ...
    pub fn new(
        ...
        let device_id_filter = match std::env::var("NEOZED_DEVICE_ID") {
            Ok(val) => parse_pci_id(&val)
                .context("Failed to parse device ID from `NEOZED_DEVICE_ID` environment variable")
                .log_err(),
            Err(std::env::VarError::NotPresent) => None,
            err => {
                err.context("Failed to read value of `NEOZED_DEVICE_ID` environment variable")
                    .log_err();
                None
            }
        };
    ...
    async fn select_adapter_and_device(
        ...
        if let Some(device_id) = device_id_filter {
            log::info!("NEOZED_DEVICE_ID filter: {:#06x}", device_id);
        }

        // Sort adapters into a single priority order. Tiers (from highest to lowest):
        //
        // 1. NEOZED_DEVICE_ID match — explicit user override
