...
//!
//! Used by Neo Zed's OAuth-based sign-in flows (e.g. MCP servers, ChatGPT
...
/// layout styled to match Neo Zed's dark theme. The `title` is rendered as a
                ...
                let (status_code, body) = match &result {
                    Ok(_) => (
                        ...
                        oauth_callback_page(
                            ...
                            "You can close this tab and return to Neo Zed.",
                            false,
                        ),
                    ),
                    Err(err) => {
