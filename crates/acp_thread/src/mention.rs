    ...
    pub fn to_uri(&self) -> Url {
        match self {
            ...
            MentionUri::PastedImage { name } => {
                let mut url = Url::parse("neozed:///agent/pasted-image").unwrap();
            ...
            MentionUri::TerminalSelection { line_count } => {
                let mut url = Url::parse("neozed:///agent/terminal-selection").unwrap();
                url.query_pairs_mut()
                    .append_pair("lines", &line_count.to_string());
                url
            }
            MentionUri::GitDiff { base_ref } => {
                let mut url = Url::parse("neozed:///agent/git-diff").unwrap();
                url.query_pairs_mut().append_pair("base", base_ref);
                url
            }
            MentionUri::MergeConflict { file_path } => {
                let mut url = Url::parse("neozed:///agent/merge-conflict").unwrap();
...
mod tests {
    ...
    fn test_parse_untitled_selection_uri() {
        let selection_uri = uri!("neozed:///agent/untitled-buffer#L1:10");
        let parsed = MentionUri::parse(selection_uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Selection {
                abs_path: None,
    ...
    fn test_parse_thread_uri() {
        let thread_uri = "neozed:///agent/thread/session123?name=Thread+name";
    ...
    fn test_parse_rule_uri() {
        let rule_uri =
            "neozed:///agent/rule/d8694ff2-90d5-4b6f-be33-33c1763acd52?name=Some+rule";
    ...
    fn test_parse_diagnostics_uri() {
        let uri = "neozed:///agent/diagnostics?include_warnings=true";
    ...
    fn test_parse_diagnostics_uri_warnings_only() {
        let uri = "neozed:///agent/diagnostics?include_warnings=true&include_errors=false";
        let parsed = MentionUri::parse(uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Diagnostics {
                include_errors,
    ...
    fn test_invalid_zed_path() {
        assert!(MentionUri::parse("neozed:///invalid/path", PathStyle::local()).is_err());
        assert!(MentionUri::parse("neozed:///agent/unknown/test", PathStyle::local()).is_err());
    }
    ...
    fn test_parse_terminal_selection_uri() {
        let terminal_uri = "neozed:///agent/terminal-selection?lines=42";
        ...
        // Test single line
        let single_line_uri = "neozed:///agent/terminal-selection?lines=1";
