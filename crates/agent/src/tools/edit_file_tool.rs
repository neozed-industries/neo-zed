...
";
    ...
    async fn test_streaming_authorize(cx: &mut TestAppContext) {
        ...
        let _auth =
            cx.update(|cx| tool.authorize(&PathBuf::from(".neozed/settings.json"), &stream_tx, cx));

        let event = stream_rx.expect_authorization().await;
        assert_eq!(
            event.tool_call.fields.title,
            Some("Edit `.neozed/settings.json` (local settings)".into())
        );
        ...
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        let _auth =
            cx.update(|cx| tool.authorize(&PathBuf::from("root/.neozed/tasks.json"), &stream_tx, cx));
        let event = stream_rx.expect_authorization().await;
        assert_eq!(
            event.tool_call.fields.title,
            Some("Edit `root/.neozed/tasks.json` (local settings)".into())
        );
    ...
    async fn test_streaming_needs_confirmation_with_different_modes(cx: &mut TestAppContext) {
        ...
        for _mode in modes {
            ...
            let _auth = cx.update(|cx| {
                tool.authorize(&PathBuf::from("project/.neozed/settings.json"), &stream_tx, cx)
            });
