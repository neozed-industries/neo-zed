...
/// `.neozed/settings.json` where `.neozed` is a symlink to an external directory.
///
/// Note: intermediate directories *can* be symlinks (not just leaf entries),
/// so we must walk the full ancestor chain. For example:
///   `ln -s /external/config /project/.neozed`
/// makes `.neozed` an intermediate symlink directory.
...
/// either inside a `.neozed/` local-settings directory, inside `.agents/skills/`, or inside
...
/// `.neozed/` and `.agents/skills/` protections against the canonical form
...
///   2. Intra-project symlinks, e.g. a symlink `safe -> .neozed` followed
...
/// like `~/projects/.neozed/foo` doesn't classify every file inside it as
/// `.neozed/` local-settings — only files that have `.neozed` (or
