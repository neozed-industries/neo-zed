...
/// SettingsObserver observers changes to .neozed/{settings, task}.json files in local worktrees
/// (or the equivalent protobuf messages from upstream) and updates local settings
/// and sends notifications downstream.
/// In ssh mode it also monitors ~/.config/neozed/{settings, task}.json and sends the content
/// upstream.
