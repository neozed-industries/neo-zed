...
impl<T: InventoryContents> InventoryFor<T> {
    fn worktree_scenarios(
        ...
        let has_zed_dir = worktree_dirs
            .map(|dirs| {
                dirs.keys()
                    .any(|dir| dir.file_name().is_some_and(|name| name == ".neozed"))
            })
            .unwrap_or(false);
...
pub enum TaskSourceKind {
    ...
    UserInput,
    /// Tasks from the worktree's .neozed/task.json
    Worktree {
        id: WorktreeId,
        directory_in_worktree: Arc<RelPath>,
        id_base: Cow<'static, str>,
    },
    /// ~/.config/neozed/task.json - like global files with task definitions, applicable to any path
    AbsPath {
        id_base: Cow<'static, str>,
        abs_path: PathBuf,
    },
