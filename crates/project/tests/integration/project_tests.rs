...
async fn test_editorconfig_support(cx: &mut gpui::TestAppContext) {
    ...
    // .editorconfig overrides .neozed/settings
    ...
    // When max_line_length is "off", default to .neozed/settings.json
...
async fn test_git_provider_project_setting(cx: &mut gpui::TestAppContext) {
    ...
    fs.atomic_write(
        Path::new(path!("/dir/.neozed/settings.json")).to_owned(),
        "{}".into(),
    )
    .await
    .unwrap();
...
async fn test_managing_project_specific_settings(cx: &mut gpui::TestAppContext) {
    ...
    let topmost_local_task_source_kind = TaskSourceKind::Worktree {
        id: worktree_id,
        directory_in_worktree: rel_path(".neozed").into(),
        id_base: "local worktree tasks from directory \".neozed\"".into(),
    };
        ...
        vec![
            (
                TaskSourceKind::Worktree {
                    id: worktree_id,
                    directory_in_worktree: rel_path("b/.neozed").into(),
                    id_base: "local worktree tasks from directory \"b/.neozed\"".into()
                },
...
async fn test_invalid_local_tasks_shows_toast_with_doc_link(cx: &mut gpui::TestAppContext) {
    ...
    // We need to start with a valid `.neozed/tasks.json` file as otherwise the
    ...
    // Update the `.neozed/tasks.json` file with an invalid variable, so we can
    // later assert that the `Event::Toast` even is emitted.
    fs.save(
        path!("/dir/.neozed/tasks.json").as_ref(),
