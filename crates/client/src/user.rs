...
impl UserStore {
    ...
    pub fn plan(&self) -> Option<Plan> {
        ...
        if let Ok(plan) = std::env::var("NEOZED_SIMULATE_PLAN").as_ref() {
            ...
            return match plan.as_str() {
                ...
                _ => {
                    panic!("NEOZED_SIMULATE_PLAN must be one of 'free', 'trial', or 'pro'");
                }
            };
        }
    ...
    fn update_authenticated_user(
        ...
        let staff = response.user.is_staff && !*feature_flags::NEOZED_DISABLE_STAFF;
