...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("NeoZed.exe"), p("old\\NeoZed.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\NeoZed.exe"), p("old\\bin\\NeoZed.exe")),
        ...
        // Copy new files
        Job::move_file(p("install\\NeoZed.exe"), p("NeoZed.exe")),
        Job::move_file(p("install\\bin\\NeoZed.exe"), p("bin\\NeoZed.exe")),
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("NeoZed.exe"),
        app_dir.join("bin\\NeoZed.exe"),
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>, launch: bool) -> Result<()> {
    ...
    if launch {
        ...
        let _ = std::process::Command::new(app_dir.join("NeoZed.exe")).spawn();
    }
