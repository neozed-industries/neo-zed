...
use language::{BufferId, ClientCommand, default_client_command};
...
fn group_lenses_by_row(
    ...
    for (position, item) in lenses {
        ...
        let indent = snapshot.indent_size_for_line(MultiBufferRow(row)).len;
        let indent_anchor = snapshot.anchor_before(Point::new(row, indent));
        grouped
            .entry(MultiBufferRow(row))
            .or_insert_with(|| (indent_anchor, Vec::new()))
            .1
            .push(item);
    }
    ...
    move |cx| {
        ...
        for (i, item) in lens.items.iter().enumerate() {
            ...
            let element = div()
                ...
                .when_some(item.action.clone(), |lens_element, code_action| {
...
pub(super) fn try_handle_client_command(
    ...
    let client_command = project
        ...
        .and_then(|adapter| adapter.adapter.client_command(&command.command))
        .or_else(|| default_client_command(&command.command));

    match client_command {
        Some(ClientCommand::ScheduleTask) => {
            try_schedule_task(command, action, editor, workspace, window, cx)
        }
...
fn try_schedule_task(
    ...
    let project = workspace.read(cx).project().clone();
    let task_template = project
        .read(cx)
        .lsp_store()
        .read(cx)
        .language_server_adapter_for_id(action.server_id)
        .and_then(|adapter| adapter.adapter.command_to_task(command));

    let Some(task_template) = task_template else {
        return false;
    };
                ...
                match result {
                    Ok(Some(actions)) => {
                        let individual_lenses: Vec<(Anchor, CodeLensItem)> = actions
                            .into_iter()
                            .filter_map(|action| {
                                let (title, is_actionable) = match &action.lsp_action {
                                    project::LspAction::CodeLens(lens) => match &lens.command {
                                        Some(cmd) => {
                                            let actionable = !cmd.command.is_empty();
                                            (SharedString::from(&cmd.title), actionable)
                                        }
                                        None => return None,
                                    },
                                    _ => return None,
                                ...
                                let action = if is_actionable { Some(action) } else { None };
            ...
            editor
                .update(cx, |editor, cx| {
                    ...
                    let mut all_new_blocks: Vec<(BufferId, Vec<BlockProperties<Anchor>>)> =
                        Vec::new();
                    for (buffer_id, lense_lines) in new_lenses_per_buffer {
                        if lense_lines.is_empty() {
                            continue;
                        }
                        let blocks: Vec<BlockProperties<Anchor>> = lense_lines
                            .into_iter()
                            .enumerate()
                            .map(|(line_number, lens_line)| {
                                let position = lens_line.position;
                                let render_fn = render_code_lens_line(
                                    line_number,
                                    lens_line,
                                    editor_handle.clone(),
                                );
