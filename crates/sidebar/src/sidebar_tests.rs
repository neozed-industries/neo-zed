            ...
            .map(|(ix, entry)| {
                ...
                match entry {
                    ...
                    ListEntry::Thread(thread) => {
                        ...
                        let notified = if thread
                            .metadata
                            .session_id
                            .as_ref()
                            .is_some_and(|sid| sidebar.contents.is_thread_notified(sid))
                        {
                            " (!)"
                        } else {
                            ""
    ...
    let parent_status = sidebar.read_with(cx, |sidebar, _cx| {
        sidebar
            ...
            .find_map(|entry| match entry {
                ListEntry::Thread(thread)
                    if thread.metadata.session_id == Some(parent_session_id.clone()) =>
                {
                    Some(thread.status)
                }
                _ => None,
...
async fn test_archived_threads_excluded_from_sidebar_entries(cx: &mut TestAppContext) {
    ...
    cx.update(|_, cx| {
        ThreadMetadataStore::global(cx)
            .update(cx, |store, cx| store.archive(archived_thread_id, None, cx))
    });
    cx.run_until_parked();
...
mod property_test {
    ...
    fn verify_all_threads_are_shown(sidebar: &Sidebar, cx: &App) -> anyhow::Result<()> {
        ...
        for group_key in mw.project_group_keys() {
            ...
            {
                if let Some(sid) = metadata.session_id.as_ref() {
                    metadata_thread_ids.insert(sid.clone());
                }
            }
