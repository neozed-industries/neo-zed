...
const LIGHT_THEMES: [&str; 3] = ["Neo Zed Light", "One Light", "Ayu Light"];
const DARK_THEMES: [&str; 3] = ["Neo Zed Dark", "One Dark", "Ayu Dark"];
const FAMILY_NAMES: [SharedString; 3] = [
    SharedString::new_static("NeoZed"),
    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Help improve Neo Zed by sending anonymous usage data".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Help fix Neo Zed by sending crash reports so we can fix critical issues fast"
                        .into(),
                ),
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "Neo Zed can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("Trust All Projects By Default"),
        Some(
            "Automatically mark all new projects as trusted to unlock all Neo Zed features".into(),
        ),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
