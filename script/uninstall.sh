...
# Uninstalls Neo Zed that was installed using the install.sh script

check_remaining_installations() {
    platform="$(uname -s)"
    if [ "$platform" = "Darwin" ]; then
        # Check for any Neo Zed variants in /Applications
        remaining=$(ls -d /Applications/Neo\ Zed*.app 2>/dev/null | wc -l)
        [ "$remaining" -eq 0 ]
    else
        # Check for any Neo Zed variants in ~/.local
        remaining=$(ls -d "$HOME/.local/neozed"*.app 2>/dev/null | wc -l)
        [ "$remaining" -eq 0 ]
    fi
}

prompt_remove_preferences() {
    printf "Do you want to keep your Neo Zed preferences? [Y/n] "
    read -r response
    case "$response" in
        [nN]|[nN][oO])
...
main() {
    ...
    echo "Neo Zed has been uninstalled"
}
...
macos() {
    ...
    # Remove the database directory for this channel
    rm -rf "$HOME/Library/Application Support/Neo Zed/db/0-$db_suffix"
    ...
    # Remove the entire Neo Zed directory if no installations remain
    if check_remaining_installations; then
        rm -rf "$HOME/Library/Application Support/Neo Zed"
        rm -rf "$HOME/Library/Logs/Neo Zed"
