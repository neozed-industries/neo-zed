...
> **Note:** The built-in `search_web` tool is only available to [Neo Zed Pro](https://zed.dev/pricing) subscribers using the Neo Zed AI provider. If you're on a free plan or using a different provider, you can get equivalent functionality by connecting an MCP server that provides web search capabilities. See [MCP servers](./mcp.md) for details.
